...
// Recupera la posizione live della barca
function getLivePosition($conn, $targa) {
    $res = pg_query_params($conn, "SELECT ts, lat, lon, id_rotta FROM boats_current_position WHERE targa_barca = $1", [$targa]);
    if ($res && pg_num_rows($res) > 0) {
        $row = pg_fetch_assoc($res);
    ...
    // Se non esiste, recupera la posizione del faro
...
// Determina lo stato della barca (nel porto o in mare)
